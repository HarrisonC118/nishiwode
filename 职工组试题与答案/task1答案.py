...
def plot_and_denoise(gray_img, save_prefix: Path):
    # TODO: 图像去噪 - 绘制原图直方图
    # 要求：使用matplotlib绘制直方图，保存原图与去噪结果，需有截图
    plt.figure()
...
def required_filter(img_gray):
    ...
    # 要求：使用cv2.blur进行均值滤波，核大小为3x3
    img_gray_f = cv2.blur(img_gray, (3,3))
    return img_gray_f
...
def process_one(img_path: str, out_img_path: Path, is_test: bool):
    ...
    if is_test:
        ...
        # 要求：使用cv2.morphologyEx对测试集图片进行形态学开运算
    ...
    # TODO: 数据增强 - 旋转操作并保存
    # 要求：使用cv2.getRotationMatrix2D和cv2.warpAffine进行15度旋转，并按照命名规范保存
    h, w = denoised.shape
    center = (w // 2, h // 2)
    rotation_matrix = cv2.getRotationMatrix2D(center, 15, 1.0)
    rotated = cv2.warpAffine(denoised, rotation_matrix, (w, h))
    cv2.imwrite(str(out_img_path.with_name(out_img_path.stem + "_denoised.png")), denoised)
    cv2.imwrite(str(out_img_path.with_name(out_img_path.stem + "_rotated.png")), rotated)
    return True
