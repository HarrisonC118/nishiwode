...
class SizeDataset(Dataset):
    ...
    def __init__(self, list_txt: Path):
        ...
        with open(list_txt, "r", encoding="utf-8") as f:
            for line in f:
                p, t = line.rstrip(......).split("\t", 1)
                self.samples.append((......, ......))  # 提示：去除行尾空白字符并存储图片路径和标签
    ...
    def __getitem__(self, idx):
        ...
        return ......, target, ......  # 提示：返回图片tensor、标签tensor和标签长度
...
class SizeCRNN(nn.Module):
    def __init__(self, nclass):
        super().__init__()
        # TODO: 模型搭建 - 构建CNN卷积层
        # 要求：实现卷积层(32/64/128/256) + 池化层(2×2) + 全连接层
        self.cnn = nn.Sequential(
            nn.Conv2d(......), nn.BatchNorm2d(32), nn.ReLU(), nn.MaxPool2d(2,2),
            nn.Conv2d(32, ......, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(), nn.MaxPool2d(2,2),
            nn.Conv2d(64, ......, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU(),
            nn.Conv2d(128,128,3,1,1), nn.BatchNorm2d(128), nn.ReLU(), nn.MaxPool2d((2,1)),
            nn.Conv2d(128, ......, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # 提示：CNN输出形状是(B,C,H,W)，LSTM需要(B,T,C)格式，需要squeeze压缩H维度并permute转换维度
        x = self.cnn(x)
        x = self.att_blocks(x)
        x = x.......permute(0, 2, 1)
        x, _ = self.rnn(x)
        x = self.dropout(x)
        x = self.......  # 提示：使用全连接层进行分类，观察__init__中定义的self.fc层
...
def evaluate(model, loader, criterion, use_beam=False, debug=False):
    ...
    return ap, val_loss
def train_model():
    train_ds, val_ds = SizeDataset(TRAIN_TXT), SizeDataset(VAL_TXT)
    ...
    # 提示：训练轮数范围，参考第16行的EPOCHS配置
    for epoch in tqdm(range(......, ......), desc="Training"):
        model.train()
        ...
        torch......(model.state_dict(), SAVE_PATH)
