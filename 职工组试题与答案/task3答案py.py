...
def recognize_single():
    ...
    with torch.no_grad():
        ...
        text = decode_greedy(seq)
    
    # TODO: 显示识别结果
    # 要求：使用config方法更新result_label显示识别结果，更新status_label显示完成状态
    result_label.config(text=f"Size: {text}")
...
def batch_recognize():
    ...
    folder = filedialog.askdirectory(title="Select folder containing size images")
...
def create_gui():
    ...
    img_label = Label(left_frame, text="Please upload a size image", font=("Arial", 10), bg="#e8f8f5", width=50, height=15)
    ...
    upload_btn = Button(right_frame, text="Upload Size Image", command=upload_image,
                        bg="#1abc9c", fg="white", font=("Arial", 10, "bold"), width=18, height=2)
    upload_btn.pack(pady=5)
    
    # TODO: 界面功能 - 创建识别按钮
    # 要求：创建Button组件，绑定recognize_single函数，设置文本为"Recognize Size"
    recognize_btn = Button(right_frame, text="Recognize Size", command=recognize_single,
